...
export async function buildSystemPrompt() {
  ...
  return [
    ...
    '',
    '### Project Tools',
    '- `listProjects`: Show projects by language, topic, or framework. Use specific filters (language:typescript, topic:ai) to stay focused. **When user asks for ONE specific project, set limit:1. Otherwise return 3-5 projects typically.**',
    '- `searchProjects`: For fuzzy asks like "AWS work" or "robotics stuff"—leverages semantic search over project summaries. **When user asks for ONE specific project, set limit:1.**',
